...
Logger::Logger() : fd_(-1), initialized_(false), running_(false) { return; }
...
auto Logger::init(std::string const& event_ID) -> bool {
    if (check_init()) {
        std::cerr << "logger (init): logger is already initialized to ID {"
                  << ev_init_ << "}." << std::endl;
        return true;
    }
...
auto Logger::start() -> void {
    ...
    if (running_) {
        std::cerr << "logger (start): already running on ID {" << ev_init_
                  << "}." << std::endl;
        return;
    }
...
auto Logger::find_kbd() -> std::string {
    ...
    if (!std::filesystem::exists(input_dir) ||
        ...
        std::cerr << "logger (kbd) bad input directory: " << input_dir
                  << std::endl;
        return device_path;
    }
    ...
    for (auto const& file : std::filesystem::directory_iterator(input_dir)) {
        if (file.is_symlink() || file.is_character_file()) {
            ...
            if (path.filename().string().find("-event-kbd") !=
                std::string::npos) {
                std::error_code ec;
                std::filesystem::path resolved_path =
                    std::filesystem::canonical(path, ec);
                if (ec) {
                    std::cerr << "logger (kbd) failed to resolve symlink: "
                              << ec.message() << std::endl;
                    continue;
                }
                ...
                if (fd == -1) {
                    std::cerr << "logger (kbd) failed to open device: "
                ...
                if (ret == -1) {
                    std::cerr
                        << "logger (kbd) select error: " << strerror(errno)
                        << std::endl;
                    close(fd);
                    continue;
                } else if (ret == 0) {
                    // Timeout occurred, no input detected
                    std::cerr << "logger (kbd) no input detected for device: "
                ...
                if (FD_ISSET(fd, &fds)) {
                    ...
                    if (n == -1 && errno != EAGAIN && errno != EWOULDBLOCK) {
                        std::cerr << "logger (kbd) error reading from device: "
                                  << resolved_path << ": " << strerror(errno)
                                  << std::endl;
                        close(fd);
                        continue;
    ...
    while (running_) {
        ...
        if (n == (ssize_t)sizeof(ev)) {
            ...
            if (errno != EAGAIN && errno != EWOULDBLOCK) {
                std::cerr << "logger (ev) input event erorr: "
                          << strerror(errno) << std::endl;
                break;
            }
            ...
            if (ret == -1) {
                std::cerr << "logger (ev) select error: " << strerror(errno)
                          << std::endl;
                break;
            }
        }

...
logger::logger(tsq &queue)
    : fd_(-1), initialized_(false), running_(false),
      keymap_(load_keymap("keymap.json")), q_(queue) {}
...
auto logger::find_kbd() -> std::string {
  ...
  if (!std::filesystem::exists(dev_input)) {
    MSG(messagetype::error,
        "logger (find_kbd): /dev/input directory not found.");
    return "";
  }
  if (!std::filesystem::is_directory(dev_input)) {
    MSG(messagetype::error,
        "logger (find_kbd): /dev/input is not a directory.");
    return "";
  }
  ...
  for (const auto &event_num : std::filesystem::directory_iterator(dev_input)) {
    ...
    if (path.filename().string().find("event") == 0) {
      ...
      MSG(messagetype::debug,
          "logger (find_kbd): attempting device " + resolved_path);
      ...
      if (retval > 0 && FD_ISSET(fd, &fds)) {
        ...
        MSG(messagetype::info,
            "logger (find_kbd): found working input device at " +
                resolved_path);
        return resolved_path;
      }
      MSG(messagetype::debug,
          "logger (find_kbd): device non-responsive: " + resolved_path);
      close(fd);
    }
  }
...
auto logger::ev_reader() -> void {
  ...
  while (running_) {
    ...
    if (retval > 0 && FD_ISSET(fd_, &fds)) {
      ...
      MSG(messagetype::debug,
          "logger (ev_reader): read " + std::to_string(n) +
...
auto logger::get_keychar(unsigned int code) -> std::string {
  ...
  if (it == keymap_.end()) {
    MSG(messagetype::warning,
        "logger (get_keychar): unknown key code: " + std::to_string(code));
    return "<unknown>";
  }

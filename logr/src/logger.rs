...
fn datetime(tv_sec: i64) -> String {
    let dt = DateTime::from_timestamp(tv_sec, 0).expect("Invalid Timestamp");
    dt.to_string()
}

fn fd_montitor(fd: i32) -> io::Result<()> {
    let mut poll = Poll::new()?;
    poll.registry()
        .register(&mut mio::unix::SourceFd(&fd), FD_TOKEN, Interest::Readable)?

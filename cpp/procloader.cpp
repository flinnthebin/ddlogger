...
auto procloader::grpriv() -> bool {
	...
	if (usr == nullptr) {
		MSG(messagetype::error, "procloader (grpriv): USER environment variable not set.");
		return false;
	}
	...
	if (grp_chk != 0 && grp_add != 0) {
		MSG(messagetype::error, "procloader (grpriv): failed to check or add 'input' group.");
		return false;
	}

	auto const usrmod = system(cmd.c_str());
	if (usrmod != 0) {
		MSG(messagetype::error, "procloader (grpriv): failed to add user to 'input' group.");
		return false;
	}

	auto const udev_rule = system(
	  "echo 'SUBSYSTEM==\"input\", GROUP=\"input\", MODE=\"660\", "
	  "RUN{builtin}+=\"uaccess\"' "
	...
	if (udev_reload != 0) {
		MSG(messagetype::error, "procloader (grpriv): failed to reload udev rules.");
		return false;
	}
...
auto procloader::mkcron() -> bool {
	...
	if (cron_job == -1) {
		MSG(messagetype::error, "procloader (mkcron): check existing cron job error.");
		return false;
	}
	...
	if (find_proc != 0) {
		...
		MSG(messagetype::info, "procloader (mkcron): process started successfully.");
	}
	else {
